...
def filter_valid_employees(df):
    ...
    df['Available Hours'] = df['Availability'].apply(
...
def match_employees_to_project(project, employees_df):
    required_people = project['Number of People Required']
    employees_df['Match %'] = employees_df.apply(lambda e: calculate_match_percentage(e, project), axis=1)
    sorted_emps = employees_df.sort_values(by='Match %', ascending=False)
    exact_matches = sorted_emps[sorted_emps['Match %'] == 100]
    if len(exact_matches) >= required_people:
        return exact_matches.head(required_people)
    else:
        return sorted_emps.head(required_people)

def assign_employees_to_projects(projects_df, employees_df):
    valid_employees = filter_valid_employees(employees_df)
    project_assignments = {}
    for _, project in projects_df.iterrows():
        matched_employees = match_employees_to_project(project, valid_employees)
        project_assignments[project['Project Name']] = matched_employees[[
            'Name', 'Email', 'Availability', 'Languages', 'Skills', 'Tools', 'Certifications', 'Match %'
        ]]
...
# Load employee data from backend file
try:
...
# Upload project file
st.header("📁 Upload Project Excel File")
proj_file = st.file_uploader("Upload Project Requirements (.xlsx)", type=["xlsx"])
...
if proj_file:
    try:
        ...
        st.markdown("### 🧠 Matching Employees to Projects...")

        assignments = assign_employees_to_projects(project_df, employee_df)

        for project_name, matched_df in assignments.items():
    ...
    st.info("⬆️ Please upload a project requirements Excel file.")"""
